...
- **Vercel**: `scripts/build-api.mjs` bundles the API via esbuild into `api/handler.mjs` (serverless function). Frontend builds to `packages/web/dist`. `vercel.json` sets `buildCommand`, `outputDirectory`, function config, and SPA rewrite.
  ...
  - **⚠ Schema changes (CRITICAL):** the Vercel build runs `scripts/migrate.ts`, but it **silently skips when `DATABASE_URL` is absent from the *build* env** — so a deploy can ship code that SELECTs new columns against an un-migrated DB → every query on that table 500s (this caused the 2026-06 `downtime_categories` outage). Adding a column to `schema.ts` is **not** enough. Before merging schema-dependent code: apply the migration to prod first — `DATABASE_URL="<prod>" pnpm db:push` (columns are additive/backward-compatible, so "DB first, then code" never breaks the running version) — **or** ensure `DATABASE_URL` is set for the Vercel *Build* environment (not just Runtime).
