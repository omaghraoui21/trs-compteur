      ...
      {activeSession && activeSession.status === "active" && (
        <div className="bg-white rounded-2xl border shadow-sm p-5 mb-4">
          <div className="text-center">
            <div className="text-[11px] uppercase tracking-wider text-gray-500 mb-1">Session ouverte</div>
          ...
          <div className="grid grid-cols-2 gap-3 mt-4">
            <div className="rounded-xl bg-gray-50 p-3">
              <div className="text-[11px] text-gray-500 mb-0.5">Lot en cours</div>
              {activeLot ? (
                <>
                ...
                <div className="text-sm text-gray-500 font-medium">Aucun lot actif</div>
              )}
            </div>
            <div className={`rounded-xl p-3 transition-colors duration-300 ${activeLot ? "bg-green-50" : "bg-gray-50"}`}>
              <div className="text-[11px] text-gray-500 mb-0.5">Activité</div>
        ...
        <>
          ...
          <div className="bg-white rounded-xl border shadow-sm mb-4">
            <div className="px-4 py-3 border-b flex items-center justify-between">
              ...
              <span className="text-xs text-gray-500">{detail.events.length} événements</span>
            </div>
                ...
                return (
                  <div key={ev.id} className="px-4 py-2.5 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {product && <div className="text-xs text-gray-500 truncate">{product.name}</div>}
                    </div>
                    ...
                    <div key={lot.id} className="px-4 py-3">
                      <div className="flex items-center justify-between gap-2 mb-1">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          {product && <span className="text-gray-500 text-xs truncate">{product.name}</span>}
                          {lotDuration && (
                      ...
                      <div className="flex items-center gap-3 text-xs text-gray-500 flex-wrap">
                        ...
                        {lotTrs && (
                          <span className="ml-auto inline-flex items-center gap-1.5">
                            <span className="text-xs font-medium text-gray-500">TP {fmtPct(lotTrs.TP)}</span>
          ...
          <div key={dt.id} className="flex items-center gap-2 border rounded-lg px-3 py-2 bg-gray-50">
            {confirmId === dt.id ? (
              ...
              <>
                ...
                <span className="text-xs text-gray-500 shrink-0">{dt.famille}</span>
                ...
                <button onClick={() => setConfirmId(dt.id)} aria-label="Supprimer"
                  className="p-2 rounded text-gray-500 hover:text-red-500 transition shrink-0">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border p-4 space-y-4">
        ...
        <div>
          <label htmlFor="dt-comment" className="block text-sm font-medium mb-1">
            Commentaire
            <span className="ml-1.5 text-xs text-gray-500 font-normal">(raison obligatoire si modification — Annex 11)</span>
          </label>
      ...
      >
        ...
        <div className="mb-4">
          <label htmlFor="eos-notes" className="block text-xs font-medium text-gray-600 mb-1">
            Remarques de fin de poste <span className="font-normal text-gray-500">(facultatif — tracé dans l'audit GMP)</span>
          </label>
          <textarea
            id="eos-notes"
            value={notes}
